...
export default function DiatExamPage() {
  return (
    <div className="flex flex-col">
      <div className="bg-brand-navy text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">DIAT 시험센터</h1>
          <p className="text-blue-100 text-lg max-w-2xl mx-auto">디지털정보활용능력(DIAT) 공식 자격검정장입니다.</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-12">
            ...
            <div className="p-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {['프리젠테이션', '스프레드시트', '워드프로세서', '정보통신상식'].map((subject) => (
                      <div key={subject} className="bg-slate-50 p-4 rounded text-center border border-slate-100">
                          <span className="block font-bold text-brand-blue mb-1">{subject}</span>
                      </div>
                    ))}
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
             <div>
                 ...
                 <ul className="space-y-4">
                     {[
                       "시험 당일 낯선 환경으로 인한 실수를 최소화할 수 있습니다.",
                       "최신 버전의 S/W가 설치된 고사장에서 안정적으로 응시 가능합니다.",
                       "감독관 경력이 있는 강사진이 시험 유의사항을 꼼꼼히 지도합니다."
                     ].map((text, idx) => (
                 ...
                 <div className="mt-8">
                     <Link href="/course/student" className="text-brand-navy font-bold hover:underline">
